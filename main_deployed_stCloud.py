                    ...
                    for filename, data in edited_raw_dfs_dict.items():
                        ...
                        for ch in channels:
                            ...
                            try:
                                ...
                                # Calculate various noise params
            ...
            with tab3:    
                ...
                if len(all_subset_regs) > 0:
                    ...
                    st.markdown(f"<p style='color: dodgerblue; \
                                  font-size: 20px; \
                                  margin: 0;'>Best Window Fits Ranked by R<sup>2</sup></p>",
                                  unsafe_allow_html=True)
                    ...
                    rolling_reg = plot_data.compute_rolling_regression(final_x_y_valid_dict,
                                                                       all_subset_regs, 
                                                                       window_size)
                    ...
                    kde_analysis_df = plot_data.kde(rolling_reg, slope_col="Raw slope (umol/L/s)", tol_sd_mult=1.0)
...
class Analysis():
    ...
    def agtable(self, 
        ...
        for filename, channels in all_regression_results.items():
            ...
            for ch, results in channels.items():
                ...
                lower_ci = results.get("CI Low (umol/L/s)")
                upper_ci = results.get("CI High (umol/L/s)")
                ...
                slope = round(slope, 5) if isinstance(slope, (float,int)) else np.nan
                ...
                lower_ci = round(lower_ci, 5) if isinstance(lower_ci, (float, int)) else np.nan
                upper_ci = round(upper_ci, 5) if isinstance(upper_ci, (float, int)) else np.nan
        ...
        highlight_columns = ["R2", "Noise %", "slope % RSE", "Spike Score", "slope 95% CI (umol/L/s)"]
    ...
    def blank_subtraction(self,
                          all_subset_regs,
                          blank_id="Blank",
                          slope_key="slope (umol/L/s)",
                          se_key="slope stderr (umol/L/s)",
                          corrected_se_key="Corrected slope stderr (umol/L/s)",
                          coral_id_key="Coral ID",
                          ci_multiplier=1.96,
                         ):
        """
        Adds blank-corrected slopes and propagated uncertainties.
        Operates in-place on all_subset_regs.
        """
        ...
        for filename, channel_dict in all_subset_regs.items():
            ...
            # Loop through channel regression dictionaries
            for reg_data in channel_dict.values():
                
                # Get coral id
                coral_id = str(reg_data.get(coral_id_key, "")).strip().lower()

                # If this is the blank, move on.
                if coral_id == blank_id.lower():
                    blank_data = reg_data
                    break

            # If coral id is not blank, move on to blank corrections
            if blank_data is None:
                
                # No blank → populate None and skip correction
                for reg_data in channel_dict.values():
            
                    reg_data["blank-corrected slope (umol/L/s)"] = None
                    reg_data["blank-corrected slope SE (umol/L/s)"] = None
                    reg_data["blank-corrected Lag-adjusted SE (umol/L/s)"] = None
                    reg_data["blank-corrected 95% CI (umol/L/s)"] = None
                    reg_data["blank-corrected Lag-adjusted 95% CI (umol/L/s)"] = None
            ...
            for reg_data in channel_dict.values():
                ...
                # Skip blank itself
                if coral_id == blank_id.lower():
                    reg_data["blank-corrected slope (umol/L/s)"] = 0.0
                    continue

                sample_slope = reg_data.get(slope_key)
                if sample_slope is None:
                    continue

                # slope
                bc_slope = sample_slope - blank_slope
                reg_data["blank-corrected slope (umol/L/s)"] = bc_slope

                # SE (standard)
                sample_se = reg_data.get(se_key)

                # Propogate the errors for the subtraction
                if sample_se is not None and blank_se is not None:
                    # Error for the slopes without adjusted for autocorrelation ("Lag")
                    bc_se = math.sqrt(sample_se**2 + blank_se**2)

                    reg_data["blank-corrected slope SE (umol/L/s)"] = bc_se
                    reg_data["blank-corrected CI Low (umol/L/s)"] = bc_slope - ci_multiplier * bc_se
                    reg_data["blank-corrected CI High (umol/L/s)"] = bc_slope + ci_multiplier * bc_se
                    reg_data["blank-corrected 95% CI (umol/L/s)"] = (
                                                                    f"{reg_data['blank-corrected CI Low (umol/L/s)']:.6g} – "
                                                                    f"{reg_data['blank-corrected CI High (umol/L/s)']:.6g}"
                                                                    )
                ...
                if sample_corr_se is not None and blank_corr_se is not None:
                    ...
                    reg_data["blank-corrected Lag-adjusted SE (umol/L/s)"] = bc_corr_se
                    reg_data["blank-corrected Lag-adjusted CI Low (umol/L/s)"] = bc_slope - ci_multiplier * bc_corr_se
                    reg_data["blank-corrected Lag-adjusted CI High (umol/L/s)"] = bc_slope + ci_multiplier * bc_corr_se
                    reg_data["blank-corrected Lag-adjusted 95% CI (umol/L/s)"] = (
                                                                                  f"{reg_data['blank-corrected Lag-adjusted CI Low (umol/L/s)']:.6g} – "
                                                                                  f"{reg_data['blank-corrected Lag-adjusted CI High (umol/L/s)']:.6g}"
                                                                                 )
    ...
    def newey_west_analysis(self, x, y, lags, alpha=0.05):
        ...
        return {
                ...
                "Corrected slope 95% CI (umol/L/s)": nw_slope_ci,
               }
    ...
    def compute_rolling_regression(_self,
        ...
        for filename, channels in final_x_y_valid_dict.items():
            ...
            # Find blank channel for this file
            blank_slope = np.nan
            blank_se = np.nan
            blank_lag_se = np.nan
    
            if filename in all_subset_regs:
                for blank_channel, blank_data in all_subset_regs[filename].items():
                    if blank_data.get("Coral ID") == "Blank":
                        blank_slope = blank_data.get("slope (umol/L/s)", np.nan)
                        blank_se = blank_data.get("slope stderr (umol/L/s)", np.nan)
                        blank_lag_se = blank_data.get("Corrected slope stderr (umol/L/s)", np.nan)
                        break
            ...
            for channel, xy in channels.items():
                ...
                if len(x) < window_size:
                    results.append({
                                    ...
                                    "Start": np.nan,
                                    "End": np.nan,
                                    "Slope (umol/L/s)": np.nan,
                                    "R2": np.nan,
                                    "Slope SE (umol/L/s)": np.nan,
                                    "Slope %RSE": np.nan,
                                    "Slope 95% CI (umol/L/s)": "Not enough rolling windows",
                                    "intercept": np.nan,
                                    
                                    "Lag-adjusted SE (umol/L/s)": np.nan,
                                    "Lag-adjusted %RSE": np.nan,
                                    "Lag-adjusted 95% CI (umol/L/s)": "Not enough rolling windows",
                                    "System Lag (Pts)": np.nan,
                
                                    "Blank Slope Used (umol/L/s)": blank_slope,
                
                                    "blank-corrected slope (umol/L/s)": np.nan,
                                    "blank-corrected slope SE (umol/L/s)": np.nan,
                                    "blank-corrected %RSE": np.nan,
                                    "blank-corrected 95% CI (umol/L/s)": "Not enough rolling windows",
                
                                    "blank-corrected Lag-adjusted SE (umol/L/s)": np.nan,
                                    "blank-corrected Lag-adjusted %RSE": np.nan,
                                    "blank-corrected Lag-adjusted 95% CI (umol/L/s)": "Not enough rolling windows"
                                  })
                ...
                for i in range(num_windows):
                    ...
                    nw_rse = round((nw_se / abs(nw_slope)) * 100, 2) if nw_slope != 0 else np.nan
    
                    # Blank correction with error propagation
                    if not np.isnan(blank_slope):
                        blank_corrected_slope = slope - blank_slope
    
                        if not np.isnan(blank_se):
                            blank_corrected_se = np.sqrt(stderr**2 + blank_se**2)
                            blank_corrected_ci_low = blank_corrected_slope - 1.96 * blank_corrected_se
                            blank_corrected_ci_high = blank_corrected_slope + 1.96 * blank_corrected_se
                            blank_corrected_ci = f"{blank_corrected_ci_low:.6f} - {blank_corrected_ci_high:.6f}"
                            blank_corrected_rse = (
                                round((blank_corrected_se / abs(blank_corrected_slope)) * 100, 2)
                                if blank_corrected_slope != 0 else np.nan
                            )
                        else:
                            blank_corrected_se = np.nan
                            blank_corrected_ci = "NA"
                            blank_corrected_rse = np.nan
    
                        if not np.isnan(blank_lag_se):
                            blank_corrected_lag_se = np.sqrt(nw_se**2 + blank_lag_se**2)
                            blank_corrected_lag_ci_low = blank_corrected_slope - 1.96 * blank_corrected_lag_se
                            blank_corrected_lag_ci_high = blank_corrected_slope + 1.96 * blank_corrected_lag_se
                            blank_corrected_lag_ci = f"{blank_corrected_lag_ci_low:.6f} - {blank_corrected_lag_ci_high:.6f}"
                            blank_corrected_lag_rse = (
                                round((blank_corrected_lag_se / abs(blank_corrected_slope)) * 100, 2)
                                if blank_corrected_slope != 0 else np.nan
                            )
                        else:
                            blank_corrected_lag_se = np.nan
                            blank_corrected_lag_ci = "NA"
                            blank_corrected_lag_rse = np.nan
    
                    else:
                        blank_corrected_slope = np.nan
                        blank_corrected_se = np.nan
                        blank_corrected_ci = "NA"
                        blank_corrected_rse = np.nan
                        blank_corrected_lag_se = np.nan
                    ...
                    window_dict = {
                                    ...
                                    "Start": xi[0],
                                    "End": xi[-1],
                                    "Raw slope (umol/L/s)": round(slope, 4),
                                    "R2": round(r2, 4),
                                    "Raw slope SE (umol/L/s)": round(stderr, 6),
                                    "Raw slope %RSE": slope_rse,
                                    "Raw slope 95% CI (umol/L/s)": slope_ci,
                                    "intercept": round(intercept, 4),
                                    
                                    # "Slope pval": f"{nw_model.pvalues[1]:.2e}",
                                    # "SSR": round(ssr, 3),
                                    # "NW Slope (umol/L/s)":nw_slope_rnd,
                                    "Lag-adjusted 95% CI (umol/L/s)": f"{ci_low_nw:.6f} - {ci_high_nw:.6f}",
                                    # "NW pval": f"{nw_pval:.2e}",
                                    "System Lag (Pts)": lags,
    
                                    # "Blank Slope Used (umol/L/s)": blank_slope,
    
                                    "Blank-corrected slope (umol/L/s)": round(blank_corrected_slope, 6),
                                    "Blank-corrected  SE (umol/L/s)": round(blank_corrected_se, 6),
                                    "Blank-corrected %RSE": blank_corrected_rse,
                                    "Blank-corrected 95% CI (umol/L/s)": blank_corrected_ci,
                                    
                                    "Lag-adjusted SE (umol/L/s)": round(nw_se, 6),
                                    "Lag-adjusted %RSE": nw_rse,
    
                                    "Blank-corrected, Lag-adjusted SE (umol/L/s)": round(blank_corrected_lag_se, 6),
                                    "Blank-corrected, Lag-adjusted %RSE": blank_corrected_lag_rse,
                                    "Blank-corrected, Lag-adjusted 95% CI (umol/L/s)": blank_corrected_lag_ci
                                  }
    ...
    def rolling_reg_ui(self, 
        ...
        hidden_cols = [
                        ...
                        "intercept",
                        # "Slope SE (umol/L/s)",
                        # "Corrected Slope SE (umol/L/s)",
                        # # "Slope pval",
                        # "Slope 95% CI (umol/L/s)",
                        # # "SSR",
                        # "Corrected slope 95% CI (umol/L/s)",
                        "System Lag (Pts)",
                       ]
        ...
        if isinstance(selected, pd.DataFrame) and not selected.empty:
            ...
            slope, intercept = selected["Raw slope (umol/L/s)"][0], selected["intercept"][0]
    ...
    def download_regression(self, all_subset_regs):
        ...
        for filename, channels in all_subset_regs.items():
            for ch, results in channels.items():
                ...
                slope = results.get("slope (umol/L/s)", np.nan)
                slope_se = results.get("slope stderr (umol/L/s)", np.nan)
                corrected_slope_se = results.get("Corrected slope stderr (umol/L/s)", np.nan)
                slope_CI = results.get("slope 95% CI (umol/L/s)", np.nan)
                corrected_slope_CI = results.get("Corrected slope 95% CI (umol/L/s)", np.nan)
                ...
                blank_corrected_slope = results.get("blank-corrected slope (umol/L/s)", np.nan)
                blank_corrected_slope_se = results.get("blank-corrected slope SE (umol/L/s)", np.nan)
                blank_corrected_slope_CI = results.get("blank-corrected 95% CI (umol/L/s)", np.nan)
                blank_corrected_lag_se = results.get("blank-corrected Lag-adjusted SE (umol/L/s)", np.nan)
                blank_corrected_lag_CI = results.get("blank-corrected Lag-adjusted 95% CI (umol/L/s)", np.nan)
    
                # Calculate corrected slope % RSE safely
                ...
                rows.append({
                            ...
                            "Raw slope (umol/L/s)": slope,
                            "R2": r2,
                            "Raw slope SE (umol/L/s)": slope_se,
                            "Raw slope % RSE": slope_rse,
                            "Raw slope 95% CI (umol/L/s)": slope_CI,
                            
                            # Blank corrected slope info
                            "Blank-corrected slope (umol/L/s)":blank_corrected_slope,
                            "Blank-corrected SE (umol/L/s)":blank_corrected_slope_se,
                            "Blank-corrected 95% CI (umol/L/s)":blank_corrected_slope_CI,
                            
                            # Lag-adjusted slope
                            "Lag-adjusted SE (umol/L/s)": corrected_slope_se,
                            "Lag-adjusted % RSE": corrected_slope_rse,
                            "Lag-adjusted 95% CI (umol/L/s)": corrected_slope_CI,
                            
                            # Blank and lag adjusted slope stats
                            "Blank-corrected, Lag-adjusted SE (umol/L/s)":blank_corrected_lag_se,
                            "Blank-corrected, Lag-adjusted 95% CI (umol/L/s)":blank_corrected_lag_CI,
        ...
        round_5_cols = [
                        "Raw slope (umol/L/s)",
                        "Raw slope SE (umol/L/s)",
                        "Lag-adjusted SE (umol/L/s)",
                        "Blank-corrected slope (umol/L/s)"
                       ]
        ...
        # Download results
        csv = df_raw.fillna("NA").to_csv(index=False).encode("utf-8-sig")
    ...
    def kde(self, 
            ...
            slope_col: str = "Raw slope (umol/L/s)",
        ...
        if rolling_df is None or getattr(rolling_df, "empty", True):
            st.warning("Not enough data exists to calculate the most common respiration rate.")
            return pd.DataFrame()
        ...
        for (fname, ch), g in df.groupby(["Filename", "Channel"], sort=False):
            ...
            if slopes.size < int(min_slopes):
                rows.append({
                            "Filename": fname,
                            "Channel": ch,
                            # "Stable Start (s)": "Not enough rolling windows",
                            # "Stable End (s)": "Not enough rolling windows",
                            "Most Common Slope (umol/L/s)": "Not enough rolling windows",
                            "Slope Stability (STDEV)": "Not enough rolling windows",
                            # "Rate with Highest R²": "Not enough rolling windows",
                            # "Best R²": "Not enough rolling windows",
                            # "Mode − BestR² (abs)": "Not enough rolling windows",
                            # "Stable Duration (s)": "Not enough rolling windows",
                            # "Frac windows near mode": "Not enough rolling windows",
            ...
            rows.append({
                        ...
                        "Most Common Slope (umol/L/s)": mode,
                        "Slope Stability (STDEV)": sd,
        ...
        # summary["Most Common Slope (umol/L/s)"] = summary["Most Common Slope (umol/L/s)"].round(4)
        # summary["Slope Stability (STDEV)"] = summary["Slope Stability (STDEV)"].round(4)
        ...
        summary["Most Common Slope (umol/L/s)"] = summary["Most Common Slope (umol/L/s)"].apply(
                                                                                                lambda x: round(x, 4)
                                                                                                if isinstance(x, (int, float, np.number)) and np.isfinite(x)
                                                                                                else x
                                                                                               )

        summary["Slope Stability (STDEV)"] = summary["Slope Stability (STDEV)"].apply(
                                                                                      lambda x: round(x, 4)
                                                                                      if isinstance(x, (int, float, np.number)) and np.isfinite(x)
                                                                                      else x
                                                                                     )
    ...
    def kde_render(self,
                   summary,
                   title: str = "Most Common Slope",
                   button_text: str = "Download Most Common Slopes",
...
class About():
    ...
    def results_interpretation(self):
        ...
        with st.expander('🪸 Results Parameter Definitions'):
            ...
            st.markdown(
                        ...
                        **Raw (Uncorrected) Slope**
                        - **Raw slope (µmol/L/s)** → Rate of oxygen change over time (before any corrections).
                        - **R²** → Goodness of fit of the linear regression (closer to 1 = more linear).
                        - **Raw slope SE (µmol/L/s)** → Standard error of the slope (uncertainty of the estimate).
                        - **Raw slope % RSE** → Relative standard error (SE expressed as % of slope magnitude).
                        - **Raw slope 95% CI (µmol/L/s)** → Confidence interval for the slope (uncertainty range).
                        
                        ---
                        
                        **Blank-Corrected Slope**
                        - **Blank-corrected slope (µmol/L/s)** → Slope after subtracting background (blank) signal.
                        - **Blank-corrected SE (µmol/L/s)** → Standard error after blank correction.
                        - **Blank-corrected 95% CI (µmol/L/s)** → Confidence interval after blank correction.
                        
                        ---
                        
                        **Lag-Adjusted (Autocorrelation-Corrected)**
                        - **Lag-adjusted SE (µmol/L/s)** → Standard error corrected for autocorrelation (e.g., Newey–West).
                        - **Lag-adjusted % RSE** → Relative standard error after lag correction.
                        - **Lag-adjusted 95% CI (µmol/L/s)** → Confidence interval accounting for time-series dependence.
                        
                        ---
                        
                        **Blank + Lag Adjusted**
                        - **Blank-corrected, Lag-adjusted SE (µmol/L/s)** → Final uncertainty accounting for both blank correction and autocorrelation.
                        - **Blank-corrected, Lag-adjusted 95% CI (µmol/L/s)** → Most reliable confidence interval for the corrected slope.
