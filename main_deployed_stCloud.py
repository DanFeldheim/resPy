...
class Analysis():
        ...
        for filename, channel_dict in all_subset_regs.items():
            ...
            for reg_data in channel_dict.values():
                ...
                if sample_se is not None and blank_se is not None:
                    ...
                    reg_data["blank-corrected slope SE (umol/L/s)"] = None
                    reg_data["blank-corrected CI Low (umol/L/s)"] = None
                    reg_data["blank-corrected CI High (umol/L/s)"] = None
                    reg_data["blank-corrected 95% CI (umol/L/s)"] = None
                ...
                if sample_corr_se is not None and blank_corr_se is not None:
                    ...
                    reg_data["blank-corrected Lag-adjusted CI Low (umol/L/s)"] = (
                        bc_slope - ci_multiplier * bc_corr_se
                    )
                    reg_data["blank-corrected Lag-adjusted CI High (umol/L/s)"] = (
                        bc_slope + ci_multiplier * bc_corr_se
                    )
                    ...
                    reg_data["blank-corrected Lag-adjusted SE (umol/L/s)"] = None
                    reg_data["blank-corrected Lag-adjusted CI Low (umol/L/s)"] = None
                    reg_data["blank-corrected Lag-adjusted CI High (umol/L/s)"] = None
                    reg_data["blank-corrected Lag-adjusted 95% CI (umol/L/s)"] = None
            ...
            for channel, xy in channels.items():
                ...
                if len(x) < window_size:
                    results.append({
                                    ...
                                    "Start": np.nan,
                                    "End": np.nan,
                                    "Raw slope (umol/L/s)": np.nan,
                                    "R2": np.nan,
                                    "Raw slope SE (umol/L/s)": np.nan,
                                    "Raw slope %RSE": np.nan,
                                    "Raw slope 95% CI (umol/L/s)": "Not enough rolling windows",
                                    "intercept": np.nan,
                                  })
                ...
                for i in range(num_windows):
                    ...
                    window_dict = {
                                    ...
                                    "Start": xi[0],
                                    "End": xi[-1],
                                    "Raw slope (umol/L/s)": round(slope, 6),
                                    "R2": round(r2, 4),
    ...
    def rolling_reg_ui(self, 
        ...
        if isinstance(selected, pd.DataFrame) and not selected.empty:
            ...
            fig.add_trace(go.Scatter(
                                    x=x_sel[mask],
                                    y=float(slope) * x_sel[mask] + float(intercept),
                                    mode="lines",
    ...
    def download_regression(self, all_subset_regs):
        ...
        round_5_cols = [
                        ...
                        "Blank-corrected SE (umol/L/s)",
                        "Blank-corrected, Lag-adjusted SE (umol/L/s)"
                       ]
    ...
    def apply_edited_blank_selections(self,
        ...
        for _, row in edited_df.iterrows():
            ...
            if coral_id == blank_id.lower() and filename == blank_filename:
                self.clear_blank_results(results)
                results["blank-corrected slope (umol/L/s)"] = 0.0
                continue
            ...
            bc_slope = sample_slope - blank_slope
            results["blank-corrected slope (umol/L/s)"] = bc_slope
            ...
            if sample_se is not None and blank_se is not None:
                ...
                results["blank-corrected slope SE (umol/L/s)"] = bc_se
                results["blank-corrected CI Low (umol/L/s)"] = bc_slope - ci_multiplier * bc_se
                results["blank-corrected CI High (umol/L/s)"] = bc_slope + ci_multiplier * bc_se
                results["blank-corrected 95% CI (umol/L/s)"] = (
                                                                f"{results['blank-corrected CI Low (umol/L/s)']:.6g} – "
                                                                f"{results['blank-corrected CI High (umol/L/s)']:.6g}"
                                                               )
                
            else:
                results["blank-corrected slope SE (umol/L/s)"] = None
                results["blank-corrected CI Low (umol/L/s)"] = None
                results["blank-corrected CI High (umol/L/s)"] = None
                results["blank-corrected 95% CI (umol/L/s)"] = None
            ...
            if sample_corr_se is not None and blank_corr_se is not None:
                ...
                results["blank-corrected Lag-adjusted SE (umol/L/s)"] = bc_corr_se
                results["blank-corrected Lag-adjusted CI Low (umol/L/s)"] = (
                                                                             bc_slope - ci_multiplier * bc_corr_se
                                                                            )
                results["blank-corrected Lag-adjusted CI High (umol/L/s)"] = (
                                                                              bc_slope + ci_multiplier * bc_corr_se
                                                                             )
                results["blank-corrected Lag-adjusted 95% CI (umol/L/s)"] = (
                                                                             f"{results['blank-corrected Lag-adjusted CI Low (umol/L/s)']:.6g} – "
                                                                             f"{results['blank-corrected Lag-adjusted CI High (umol/L/s)']:.6g}"
                                                                            )
            
            else:
                results["blank-corrected Lag-adjusted SE (umol/L/s)"] = None
                results["blank-corrected Lag-adjusted CI Low (umol/L/s)"] = None
                results["blank-corrected Lag-adjusted CI High (umol/L/s)"] = None
                results["blank-corrected Lag-adjusted 95% CI (umol/L/s)"] = None
