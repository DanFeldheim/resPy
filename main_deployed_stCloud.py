...
class Analysis():
            ...
            for channel, xy in channels.items():
                ...
                for i in range(num_windows):
                    ...
                    window_dict = {
                                    ...
                                    "Slope SE (umol/L/s)": round(stderr, 6),
                                    "Corrected Slope SE (umol/L/s)": round(nw_se, 6),
                                    # "Slope pval": f"{nw_model.pvalues[1]:.2e}",
                                    "Slope 95% CI (umol/L/s)": slope_ci,
                                    # "SSR": round(ssr, 3),
                                    # "NW Slope (umol/L/s)":nw_slope_rnd,
                                    "Corrected slope 95% CI (umol/L/s)": f"{ci_low_nw:.6f} - {ci_high_nw:.6f}",
    ...
    def download_regression(self, all_subset_regs):
        ...
        for filename, channels in all_subset_regs.items():
            for ch, results in channels.items():
                ...
                rows.append({
                            ...
                            "slope (umol/L/s)": slope,
                            "R2": r2,
                            "slope SE (umol/L/s)": slope_se,
                            "Corrected slope SE (umol/L/s)": corrected_slope_se,
                            "slope 95% CI (umol/L/s)": slope_CI,
                            "Corrected slope 95% CI (umol/L/s)": corrected_slope_CI,
                            "slope % RSE": slope_rse,
                            "Corrected slope % RSE": corrected_slope_rse,
        ...
        if "slope % RSE" in df_display.columns:
            df_display["slope % RSE"] = df_display["slope % RSE"].apply(
                                                                        lambda x: round(float(x), 2) if pd.notna(x) else np.nan
                                                                       )
    
        if "Corrected slope % RSE" in df_display.columns:
            df_display["Corrected slope % RSE"] = df_display["Corrected slope % RSE"].apply(
                                                                                            lambda x: round(float(x), 2) if pd.notna(x) else np.nan
                                                                                           )
        ...
        st.markdown(
                    "<p style='color: Blue; font-size: 24px; margin: 0;'>Final Adjusted and Corrected Regression Results</p>",
                    unsafe_allow_html=True
                   )
