...
class Flow_Control():
    ...
    def all_calls(self):
        ...
        with tab1:
            ...
            if file_name_list:
                ...
                # Create a simple "hash" of uploaded files: just a tuple of filenames and sizes
                # This is used to make sure the warning popup box only displays when 
                # files are first uploaded
                current_hash = tuple((f.name, f.size) for f in file_name_list)
                
                # Compare current_hash filenames to uploaded filenames
                ...
                st.markdown(f"<p style='color: Blue; \
                      font-size: 24px; \
                      margin: 0;'>Linear Regression and ACF Results</p>",
                      unsafe_allow_html=True)
                ...
                st.markdown(f"<p style='color: DarkRed; \
                              font-size: 24px; \
                              margin: 0;'>Select rows to view plots and download results.</p>",
                              unsafe_allow_html=True)
    ...
    def about_model_diagnostics(self):
        ...
        with st.expander('🪸 Model Diagnostics used to test the linear regression results.'):
            ...
            st.markdown(
                        ...
                        These can be corrected with a Newey-West correction (see below).
...
class Analysis():
    ...
    def generate_plots(self, all_subset_residuals, estimate_maxlags_func, max_lag=200, pdf_filename="model_diagnostics.pdf"):
        """
