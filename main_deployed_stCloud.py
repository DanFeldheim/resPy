...
class Flow_Control():
        ...
        with tab2:    
            ...
            with tab3:    
                ...
                if len(all_subset_regs) > 0:
                    ...
                    kde_analysis_df = plot_data.kde(rolling_reg, slope_col="Raw slope (umol/L/hr)", tol_sd_mult=1.0)
    ...
    def noise_calc(self, residuals, regress, duration_s):
        ...
        slope = regress.get("slope (umol/L/hr)", np.nan)
        stderr = regress.get("slope stderr (umol/L/hr)", np.nan)
    ...
    def agtable(self, 
        ...
        for filename, channels in all_regression_results.items():
            ...
            for ch, results in channels.items():
                ...
                slope = results.get("slope (umol/L/hr)")
                lower_ci = results.get("CI Low (umol/L/hr)")
                upper_ci = results.get("CI High (umol/L/hr)")
                slope_ci = results.get("slope 95% CI (umol/L/hr)")
                R2 = results.get("R2")
                SSR = results.get("squared residuals")
                rmse = results.get("RMSE")
                pval = results.get("slope pval")
                stderr = results.get("slope stderr (umol/L/hr)")
                ...
                row = {
                        ...
                        "slope (umol/L/hr)": slope,
                        "R2": R2,
                        "slope 95% CI (umol/L/hr)": slope_ci,
                        "slope % RSE":rse_percent,
                        "lower_ci": lower_ci,
                        "upper_ci": upper_ci,
                        # "slope pval": pval_str,
                        # "p-value_numeric": pval if isinstance(pval,(float,int)) else np.nan,
                        # "slope stderr (umol/L/hr)": stderr_str,
                        # "SSR": SSR,
                        # "RMSE":rmse,
                        "Noise %":noise_percent,
                        "Spike Score":zmax,
        ...
        cell_style_jscode = JsCode("""
                                    function(params) 
                                        ...
                                        if (
                                            params.colDef.field == 'slope 95% CI (umol/L/hr)' &&
                                            params.data != null &&
        ...
        highlight_columns = ["R2", "Noise %", "slope % RSE", "Spike Score", "slope 95% CI (umol/L/hr)"]
...
So that section of your method would look like:
...
The second configure_column() call for "Noise %" and "Spike Score" should add the width settings without removing the previously assigned cellStyle.

You can adjust just the width values after seeing the result. For example, making "System Lag (Pts)" slightly narrower would be:

width=115

I would leave the other columns alone since you are already happy with them.
    ...
    def fit_regression(self, x, y):
        ...
        regress_results = {
                            ...
                            "R2":R2,
                            "slope pval":p,
                            "slope stderr (umol/L/hr)":stderr,
                            "CI Low (umol/L/hr)":ci_low,
                            "CI High (umol/L/hr)":ci_high,
                            "slope 95% CI (umol/L/hr)":slope_ci,
    ...
    def blank_subtraction(
                          ...
                          all_subset_regs,
                          blank_id="Blank",
                          slope_key="slope (umol/L/hr)",
                          se_key="slope stderr (umol/L/hr)",
                          corrected_se_key="Corrected slope stderr (umol/L/hr)",
        ...
        for filename, channel_dict in all_subset_regs.items():
            ...
            for reg_data in channel_dict.values():
                ...
                if self.is_blank_row(reg_data, coral_id_key, blank_id):
                    ...
                    reg_data["blank-corrected slope (umol/L/hr)"] = 0.0
                    reg_data["blank-corrected slope SE (umol/L/hr)"] = None
                    reg_data["blank-corrected 95% CI (umol/L/hr)"] = None
                    reg_data["blank-corrected Lag-adjusted SE (umol/L/hr)"] = None
                    reg_data["blank-corrected Lag-adjusted 95% CI (umol/L/hr)"] = None
                ...
                reg_data["blank-corrected slope (umol/L/hr)"] = bc_slope
                ...
                if sample_se is not None and blank_se is not None:
                    ...
                    reg_data["blank-corrected slope SE (umol/L/hr)"] = bc_se
                    reg_data["blank-corrected CI Low (umol/L/hr)"] = bc_slope - ci_multiplier * bc_se
                    reg_data["blank-corrected CI High (umol/L/hr)"] = bc_slope + ci_multiplier * bc_se
                    reg_data["blank-corrected 95% CI (umol/L/hr)"] = (
                                                                      f"{reg_data['blank-corrected CI Low (umol/L/hr)']:.3f} – "
                                                                      f"{reg_data['blank-corrected CI High (umol/L/hr)']:.3f}"
                                                                     )
                    ...
                    reg_data["blank-corrected slope SE (umol/L/hr)"] = None
                    reg_data["blank-corrected CI Low (umol/L/hr)"] = None
                    reg_data["blank-corrected CI High (umol/L/hr)"] = None
                    reg_data["blank-corrected 95% CI (umol/L/hr)"] = None
                ...
                if sample_corr_se is not None and blank_corr_se is not None:
                    ...
                    reg_data["blank-corrected Lag-adjusted SE (umol/L/hr)"] = bc_corr_se
                    reg_data["blank-corrected Lag-adjusted CI Low (umol/L/hr)"] = (
                                                                                   bc_slope - ci_multiplier * bc_corr_se
                                                                                  )
                    reg_data["blank-corrected Lag-adjusted CI High (umol/L/hr)"] = (
                                                                                    bc_slope + ci_multiplier * bc_corr_se
                                                                                   )
                    reg_data["blank-corrected Lag-adjusted 95% CI (umol/L/hr)"] = (
                                                                                   f"{reg_data['blank-corrected Lag-adjusted CI Low (umol/L/hr)']:.3f} – "
                                                                                   f"{reg_data['blank-corrected Lag-adjusted CI High (umol/L/hr)']:.3f}"
                                                                                  )
                
                else:
                    reg_data["blank-corrected Lag-adjusted SE (umol/L/hr)"] = None
                    reg_data["blank-corrected Lag-adjusted CI Low (umol/L/hr)"] = None
                    reg_data["blank-corrected Lag-adjusted CI High (umol/L/hr)"] = None
                    reg_data["blank-corrected Lag-adjusted 95% CI (umol/L/hr)"] = None
    ...
    def clear_blank_results(self, reg_data):
        reg_data["blank-corrected slope (umol/L/hr)"] = None
        reg_data["blank-corrected slope SE (umol/L/hr)"] = None
        reg_data["blank-corrected CI Low (umol/L/hr)"] = None
        reg_data["blank-corrected CI High (umol/L/hr)"] = None
        reg_data["blank-corrected 95% CI (umol/L/hr)"] = None
    ...
    def newey_west_analysis(self, x, y, lags, alpha=0.05):
        ...
        return {
                "Corrected slope stderr (umol/L/hr)": se_slope,
                # "NW slope pval": pval_slope,
                "Corrected slope 95% CI (umol/L/hr)": nw_slope_ci,
               }
    ...
    def compute_rolling_regression(_self,
        ...
        Rolling regression is used as a diagnostic tool to help identify a
        stable time window. The user can then crop the raw data to that window
        and calculate the final slope/blank-corrected slope separately.
        ...
        for filename, channels in final_x_y_valid_dict.items():
            ...
            for channel, xy in channels.items():
                ...
                # Get Coral ID for this file/channel
                coral_id = np.nan
                # Get the coral id from all_subset_regs
                if filename in all_subset_regs:
                    if channel in all_subset_regs[filename]:
                        coral_id = all_subset_regs[filename][channel].get("Coral ID", np.nan)
    
                # Set a size the sliding window will move each time
                step = max(1, int(window_size * (step_perc / 100.0)))
            
                # Make sure there are enough x points to create a window
                if len(x) < window_size:
                    results.append({
                                    ...
                                    "Start": np.nan,
                                    "End": np.nan,
                                    "Raw slope (umol/L/hr)": np.nan,
                                    "R2": np.nan,
                                    "Raw slope SE (umol/L/hr)": np.nan,
                                    "Raw slope %RSE": np.nan,
                                    "Raw slope 95% CI (umol/L/hr)": "Not enough rolling windows",
                                    "intercept": np.nan,
                                  })
                    
                    continue
    
                # Create rolling windows by cropping the data (shape: num_windows x window_size)
                ...
                for i in range(num_windows):
                    ...
                    # OLS fit (single fit)
                    # Convert time from seconds to hours for rate calculations
                    xi_hr = xi / 3600
                    ...
                    # Slope is now in umol/L/hr
                    slope = ols_model.params[1]
                    
                    # Intercept remains in umol/L
                    ...
                    window_dict = {
                                    ...
                                    "Start": xi[0],
                                    "End": xi[-1],
                                    "Raw slope (umol/L/hr)": round(slope, 6),
                                    "R2": round(r2, 4),
                                    "Raw slope SE (umol/L/hr)": round(stderr, 6),
                                    "Raw slope %RSE": slope_rse,
                                    "Raw slope 95% CI (umol/L/hr)": slope_ci,
    ...
    def rolling_reg_ui(self, 
        ...
        if isinstance(selected, pd.DataFrame) and not selected.empty:
            ...
            slope, intercept = selected["Raw slope (umol/L/hr)"][0], selected["intercept"][0]
    ...
    def download_regression(self, all_subset_regs):
        ...
        for filename, channels in all_subset_regs.items():
            for ch, results in channels.items():
                ...
                slope = results.get("slope (umol/L/hr)", np.nan)
                slope_se = results.get("slope stderr (umol/L/hr)", np.nan)
                corrected_slope_se = results.get("Corrected slope stderr (umol/L/hr)", np.nan)
                slope_CI = results.get("slope 95% CI (umol/L/hr)", np.nan)
                corrected_slope_CI = results.get("Corrected slope 95% CI (umol/L/hr)", np.nan)
                ...
                blank_corrected_slope = results.get("blank-corrected slope (umol/L/hr)", np.nan)
                blank_corrected_slope_se = results.get("blank-corrected slope SE (umol/L/hr)", np.nan)
                blank_corrected_slope_CI = results.get("blank-corrected 95% CI (umol/L/hr)", np.nan)
                blank_corrected_lag_se = results.get("blank-corrected Lag-adjusted SE (umol/L/hr)", np.nan)
                blank_corrected_lag_CI = results.get("blank-corrected Lag-adjusted 95% CI (umol/L/hr)", np.nan)
                ...
                rows.append({
                            ...
                            "Raw slope (umol/L/hr)": slope,
                            "R2": r2,
                            "Raw slope SE (umol/L/hr)": slope_se,
                            "Raw slope % RSE": slope_rse,
                            "Raw slope 95% CI (umol/L/hr)": slope_CI,
    
                            # Blank corrected slope info
                            "Blank-corrected slope (umol/L/hr)": blank_corrected_slope,
                            "Blank-corrected SE (umol/L/hr)": blank_corrected_slope_se,
                            "Blank-corrected 95% CI (umol/L/hr)": blank_corrected_slope_CI,
    
                            # Lag-adjusted slope
                            "Lag-adjusted SE (umol/L/hr)": corrected_slope_se,
                            "Lag-adjusted % RSE": corrected_slope_rse,
                            "Lag-adjusted 95% CI (umol/L/hr)": corrected_slope_CI,
    
                            # Blank and lag adjusted slope stats
                            "Blank-corrected, Lag-adjusted SE (umol/L/hr)": blank_corrected_lag_se,
                            "Blank-corrected, Lag-adjusted 95% CI (umol/L/hr)": blank_corrected_lag_CI,
        ...
        # High Noise (recommended threshold: >10%)
        df_raw.loc[
        ...
        round_5_cols = [
                        ...
                        "Lag-adjusted SE (umol/L/hr)",
                        "Blank-corrected slope (umol/L/hr)",
                        "Blank-corrected SE (umol/L/hr)",
                        "Blank-corrected, Lag-adjusted SE (umol/L/hr)"
                       ]
    ...
    def apply_edited_blank_selections(self,
        ...
        for _, row in edited_df.iterrows():
            ...
            if coral_id == blank_id.lower() and filename == blank_filename:
                self.clear_blank_results(results)
                results["blank-corrected slope (umol/L/hr)"] = 0.0
                continue
            ...
            bc_slope = sample_slope - blank_slope
            results["blank-corrected slope (umol/L/hr)"] = bc_slope
            ...
            if sample_se is not None and blank_se is not None:
                ...
                results["blank-corrected slope SE (umol/L/hr)"] = bc_se
                results["blank-corrected CI Low (umol/L/hr)"] = bc_slope - ci_multiplier * bc_se
                results["blank-corrected CI High (umol/L/hr)"] = bc_slope + ci_multiplier * bc_se
                results["blank-corrected 95% CI (umol/L/hr)"] = (
                                                                f"{results['blank-corrected CI Low (umol/L/hr)']:.3f} – "
                                                                f"{results['blank-corrected CI High (umol/L/hr)']:.3f}"
                                                                )
                
            else:
                results["blank-corrected slope SE (umol/L/hr)"] = None
                results["blank-corrected CI Low (umol/L/hr)"] = None
                results["blank-corrected CI High (umol/L/hr)"] = None
                results["blank-corrected 95% CI (umol/L/hr)"] = None
            ...
            if sample_corr_se is not None and blank_corr_se is not None:
                ...
                results["blank-corrected Lag-adjusted SE (umol/L/hr)"] = bc_corr_se
                results["blank-corrected Lag-adjusted CI Low (umol/L/hr)"] = (
                                                                             bc_slope - ci_multiplier * bc_corr_se
                                                                            )
                results["blank-corrected Lag-adjusted CI High (umol/L/hr)"] = (
                                                                              bc_slope + ci_multiplier * bc_corr_se
                                                                             )
                results["blank-corrected Lag-adjusted 95% CI (umol/L/hr)"] = (
                                                                             f"{results['blank-corrected Lag-adjusted CI Low (umol/L/hr)']:.3f} – "
                                                                             f"{results['blank-corrected Lag-adjusted CI High (umol/L/hr)']:.3f}"
                                                                            )
            
            else:
                results["blank-corrected Lag-adjusted SE (umol/L/hr)"] = None
                results["blank-corrected Lag-adjusted CI Low (umol/L/hr)"] = None
                results["blank-corrected Lag-adjusted CI High (umol/L/hr)"] = None
                results["blank-corrected Lag-adjusted 95% CI (umol/L/hr)"] = None
    ...
    def kde(self, 
            ...
            slope_col: str = "Raw slope (umol/L/hr)",
        ...
        for (fname, ch), g in df.groupby(["Filename", "Channel"], sort=False):
            ...
            if slopes.size < int(min_slopes):
                rows.append({
                            ...
                            "Most Common Slope (umol/L/hr)": "Not enough rolling windows",
            ...
            rows.append({
                        ...
                        "Most Common Slope (umol/L/hr)": mode,
        ...
        summary["Most Common Slope (umol/L/hr)"] = summary["Most Common Slope (umol/L/hr)"].apply(
                                                                                                lambda x: round(x, 4)
