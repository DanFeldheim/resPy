...
class Flow_Control():
    ...
    def all_calls(self):
        ...
        with tab1:
            ...
            if master_file and photo_files:
                ...
                for file in photo_files:
                    ...
                    clean_filename, date, start_time, edited_df = self.preprocess_data(file, 
                                                                                       load_files, 
                                                                                       )
                    try: 
                        # Extract RunID from filename (e.g., Run1B, Run15A), which is called Channel
                        # in the master spreadsheet
                        run_match = re.search(r"Run_?(\d+[A-Za-z])", clean_filename)
                        # This would handle names such as Run5BB.
                        # run_match = re.search(r"Run_?(\d+[A-Za-z]+)", clean_filename) 
                        
                        runID = run_match.group(1) if run_match else None
                        
                        # Separate the number from letter. The number is in the run column
                        # in the master spreadsheet. 
                        run = ''.join([c for c in runID if c.isdigit()]) if runID else None  
                        run_letter = ''.join([c for c in runID if c.isalpha()]) if runID else None 

                        L_D = "L" if "light" in clean_filename.lower() else "D"
                        
                        # st.write(date, run, run_letter, start_time, L_D)
                        
                        # Add the final raw edited_df to a dict 
                        edited_raw_dfs_dict[clean_filename] = {"data": edited_df,
                                                               "date": date,
                        ...
                        if runID is None: 
                            st.warning(
                                        f"⚠️ Could not extract RunID from filename '{clean_filename}'. "
                                        "Check naming format (e.g., Run5A, Run_12B)."  
                                      )
                            
                    except Exception as e:
                        
                        st.warning(f"⚠️ Error parsing filename '{clean_filename}': {e}")
                        
                        # Still record the file with None values so you can track it
                        edited_raw_dfs_dict[clean_filename] = {  
                                                                "data": edited_df,
                                                                "date": date,
                                                                "start_time": start_time,
                                                                "run": None,
                ...
                # Matches run, run letter, runID, time, and L_D to the master spreadsheet
                # to extract volume and coral ID
                edited_raw_dfs_dict = load_files.find_volume(edited_raw_dfs_dict, 
                                                             master_file_df,
                                                             debug = False)
                
                # st.write('before correction: ', edited_raw_dfs_dict)  
                    ...
                    for filename, data in edited_raw_dfs_dict.items():
                        ...
                        for ch in channels:
                            ...
                            try:
                                ...
                                volume = channel_metadata[ch]["Volume (mL)"]
                                coral_id = channel_metadata[ch]["Coral ID"]
                ...
                # st.write(all_regression_results)
                # st.write('after correction: ', edited_raw_dfs_dict)  
                # st.stop()
        ...
        with tab2:    
            ...
            if len(all_subset_regs) > 0:
                ...
                # To keep
                keys_to_keep = ["slope (umol/L)", "R2", "slope stderr", "slope 95% CI", "slope pval"]
                ...
                for filename, channels in all_subset_regs.items():
                    ...
                    # Create an entry for this filename in the dict created above
    ...
    def preprocess_data(self, file, load_files):
        ...
        return clean_filename, date, start_time, edited_df
...
class Load_Data():
    ...
    def find_volume(self, edited_raw_dfs_dict, master_file_df, debug=False):
        ...
        for clean_filename, file_info in edited_raw_dfs_dict.items():
            ...
            file_date = str(file_info["date"]).strip()
            
            # Deletes seconds from start time because that isn't in
            # the master spreadsheet
            file_time = str(file_info["start_time"]).strip()[:5] 
            
            file_run = str(file_info["run"]).strip()
            file_run_letter = str(file_info["run_letter"]).strip().upper()
            file_LD = str(file_info["L_D"]).strip().upper()
    
            df = file_info["data"]
            channel_metadata = {}
    
            # Loop over each channel in the DataFrame
            for ch in [c for c in df.columns if c.lower().startswith("ch")]:
                
                # Extract channel number from "Ch1" -> "1"
                ch_num = ''.join([c for c in ch if c.isdigit()])
    
                # Expected channel name in master_file_df: e.g. "1A", "2B", etc.
                master_channel = f"{ch_num}{file_run_letter}"
                
                # Normalize dates to proper datetime.date objects
                master_file_df["Date"] = pd.to_datetime(master_file_df["Date"], errors="coerce").dt.date
                file_date = pd.to_datetime(file_date, errors="coerce").date()
                
                # Match against master spreadsheet and pull out row
                match = master_file_df[
                                        (master_file_df["Date"] == file_date) &
                                        (master_file_df["Start_Time"].astype(str).str.strip().str[:5] == file_time) &
                                        (master_file_df["Run"].astype(str).str.strip() == file_run) &
                                        (master_file_df["Channel"].astype(str).str.strip().str.upper() == master_channel)
                                      ]
                 
                
                # Grab volume and coral ID
                if not match.empty:
                    
                    volume = match["Volume (mL)"].values[0]
                    coral_id = match["Coral ID"].values[0]
                    channel_metadata[ch] = {"Volume (mL)": volume, "Coral ID": coral_id}
    
                    if debug:
                        print(f"✅ Match for {clean_filename} | {ch}: {volume} mL, Coral {coral_id}")
                else:
                    
                    channel_metadata[ch] = {"Volume (mL)": None, "Coral ID": None}
                    
                    if debug:
                        print(f"❌ No match for {clean_filename} | {ch} (expected Channel={master_channel})")
    
            # Store channel mapping inside the dictionary
...
class Analysis():
    ...
    def agtable(self, 
        ...
        for filename, channels in all_regression_results.items():
            ...
            for ch, results in channels.items():
                ...
                slope = results.get("slope (umol/L)")
    ...
    def subset_dataframe(self, selected_rows, edited_raw_dfs_dict):
        ...
        if selected_rows is not None and not selected_rows.empty:
            ...
            for _, row in selected_rows.iterrows():
                ...
                # st.write(filename, channel, coral_id, volume)
    ...
    def format_regression_results(self,results_dict):
        ...
        for fname, channels in results_dict.items():
            ...
            for ch, metrics in channels.items():
                ...
                for key in metrics:
                    val = metrics[key]
                    if key in ["slope", "NW slope (umol/L)"]:
                        val = round(val, 4)
                    elif key == "R2":
                        val = round(val, 3)
    ...
    def rolling_reg_ui(self, 
        ...
        if isinstance(selected, pd.DataFrame) and not selected.empty:
            ...
            slope, intercept = selected["Slope (umol/L)"][0], selected["intercept"][0]
    ...
    def download_regression(self, all_subset_regs):
        ...
        for filename, channels in all_subset_regs.items():     
            for ch, results in channels.items():
                ...
                rows.append({
                            ...
                            "slope (umol/L)": results.get("slope (umol/L)", np.nan),
                            "slope 95% CI": slope_CI,
        ...
        df_display["slope (umol/L)"] = df_display["slope (umol/L)"].apply(
...
class Utilities():
    ...
    @staticmethod
    def match_check(master_file_df, 
                    file_date,
                    file_run,
                    file_time,
                    master_channel,
        ...
        # Check matching step-by-step
        st.write("Unique date values in master_file_df['Date']:", master_file_df["Date"].unique()[:10])

        cond_date = master_file_df["Date"].astype(str).str.strip() == file_date
        cond_time = master_file_df["Start_Time"].astype(str).str.strip().str[:5] == file_time
        cond_run = master_file_df["Run"].astype(str).str.strip() == file_run
        cond_channel = master_file_df["Channel"].astype(str).str.strip().str.upper() == master_channel
        
        st.write("---- Matching diagnostics ----")
        st.write("File date:", file_date, "→ Matches:", cond_date.sum())
        st.write("File time:", file_time, "→ Matches:", cond_time.sum())
        st.write("File run:", file_run, "→ Matches:", cond_run.sum())
        st.write("File channel:", master_channel, "→ Matches:", cond_channel.sum())
        st.write("------------------------------")
        
        # Combined filter
        match = master_file_df[cond_date & cond_time & cond_run & cond_channel]
